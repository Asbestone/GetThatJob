...
export async function runRAG(
  ...
  // pass query and all other user prompts to extractCompany
  // to get the target company name for context
  // This is important for the RAG process to work correctly
  const userMessages = chatHistory
    .filter((msg) => msg.role === "user")
    .map((msg) => msg.parts?.map((part) => part.text).join("") || "");
  const context_for_company = `${userMessages.join("\n")}\n${query}`;
  const targetCompany = await extractCompany(context_for_company);

  const similar = await zillizService.searchSimilarResumes(
    queryEmbedding,
    targetCompany,
    5
  );
  ...
  const context = similar
    .map((r) => {
      ...
      try {
        ...
        console.warn("Failed to parse skills for resume", r.id, r.skills, e);
      }
      return `Resume:\n${r.resume_text}\nSkills: ${skills.join(", ")}`;
    })
    .join("\n---\n");
  ...
  const chat = ai.chats.create({
    ...
    config: {
      maxOutputTokens: 500,
      systemInstruction: userMessageWithContext,
    },
  });
  ...
  try {
    ...
    console.log(`Chat response received: ${response.text}`); // Update context if provided by the model
    answer =
      response.text ?? "Sorry, I couldn't generate an answer at this time.";
    ...
    answer =
      "Sorry, I couldn't generate an answer based on the available data.";
  }
